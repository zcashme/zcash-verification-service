...
def _fast_match_receipt(receipts, zid: int, ignore_txids: set[str] | None = None):
    zid_tag = f"{{z:{zid}"
    ...
    for r in reversed(receipts):
        ...
        if not memo_text or zid_tag not in memo_text:
            continue
...
def _create_poll_request(sb, zid: int):
    request_id = None
    for _ in range(10):
        candidate = "".join(secrets.choice(REQUEST_ID_ALPHABET) for _ in range(REQUEST_ID_LEN))
        if not _get_poll_request(sb, candidate):
            request_id = candidate
            break
    if not request_id:
        request_id = "".join(secrets.choice(REQUEST_ID_ALPHABET) for _ in range(REQUEST_ID_LEN))
...
def verify_poll_status(request_id: str, debug_ops: bool = False):
    ...
    if receipts:
        receipts.sort(key=lambda r: r.get("blocktime") or r.get("time") or 0)
    tail = receipts[-RPC_TAIL_SIZE:] if RPC_TAIL_SIZE > 0 else receipts
    print(
        "[rpc_tail] "
        f"request_id={request_id} zid={zid} tail_size={len(tail)} "
    ...
    tail_txids = [r.get("txid") for r in tail if r.get("txid")]
    seen_txids = _get_seen_txids(sb, tail_txids)
    if seen_txids:
        ignore_txids.update(seen_txids)
        print(f"[ignore_seen_txids] zid={zid} count={len(seen_txids)}")
    fast_match = _fast_match_receipt(tail, zid, ignore_txids=ignore_txids)
    eligible = []
    if fast_match:
        ...
        matched_receipts = [r for r in tail if r.get("txid") == matched_txid]
        print(
            "[parse_store_start] "
        ...
        print(
            "[fast_match_miss] "
            f"request_id={request_id} zid={zid} receipts_checked={len(tail)}"
        )
