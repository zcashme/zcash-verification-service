...
//! ZALLET_BIN should point to the `zallet` launcher; `zallet-zebra`
...
async fn regtest_auth_payment_triggers_otp_response() {
    ...
    let lwd = Indexer::start(&lwd_bin, zebrad.rpc_port)
        .await
        .expect("start fresh lightwalletd");
    ...
    assert!(
        ...
        "expected uregtest1 address, got: {}",
        worker.service_address
    );
    assert_eq!(
        worker.otp_key_hex.len(),
        64,
        "OTP key should be 64 hex chars"
    );
    ...
    let expected =
        expected_otp(&worker.otp_key_hex, SESSION_ID, &funder_ua).expect("compute expected OTP");
